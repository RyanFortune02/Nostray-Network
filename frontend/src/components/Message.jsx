        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm mb-4 overflow-hidden">
            <div className="p-4">
                ...
                <div className="flex space-x-2 mb-2">
                    {/* Display department badges */}
                    {message.recipients && message.recipients.map((dept) => (
                        <span
                            key={dept}
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getBadgeColor(dept)}`}
                        >
                            {dept}
                        </span>
                    ))}
                ...
                <div className="text-gray-700">
                    ...
                    {isExpanded ? message.content : `${message.content.substring(0, 100)}${message.content.length > 100 ? '...' : ''}`}
                </div>
